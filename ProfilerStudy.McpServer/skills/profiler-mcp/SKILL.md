...
4. Narrow by evidence: use `analyze_frame` for a suspicious frame, `analyze_frame_detail` when hierarchical single-frame flame graph data is needed, and `analyze_time_range` for a slow-frame cluster or comparison window.
