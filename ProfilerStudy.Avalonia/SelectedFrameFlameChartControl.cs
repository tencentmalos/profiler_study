...
public sealed class SelectedFrameFlameChartControl : Control
{
	...
	public override void Render(DrawingContext context)
	{
		...
		if (rows.Count == 0)
		{
			DrawText(context, "Select a frame with scope data to show flame chart.", theme.EmptyTextBrush, 12.0, LeftPadding, TopPadding);
			return;
		}
		...
		foreach (ScopeFrameDetailRow row in rows.OrderBy(item => item.StartOffsetMs).ThenBy(item => item.Depth).Take(GetVisibleRowCount(bounds.Height)))
		{
			...
			IBrush fill = new SolidColorBrush(theme.ScopeColor(row.Depth));
			context.FillRectangle(fill, rect);
			context.DrawRectangle(null, theme.BorderPen, rect);
			if (rect.Width > 42.0)
			{
				DrawText(context, row.Name, theme.TextBrush, 10.0, rect.X + 4.0, rect.Y + 2.0);
			}
