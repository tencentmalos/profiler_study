...
public sealed class ThreadTimelineLaneControl : Control
{
	...
	public override void Render(DrawingContext context)
	{
		...
		DrawAxis(context, theme, axisLeft, axisRight, TopPadding, startFrame, endFrame);
		DrawSelection(context, theme, axisLeft, axisRight, TopPadding + AxisHeight, bounds.Height - BottomPadding, startFrame, endFrame);

		IReadOnlyList<ThreadTimelineScopeRow> rows = Rows ?? Array.Empty<ThreadTimelineScopeRow>();
		if (rows.Count == 0)
		{
			DrawText(context, "No scope timeline data in the visible frame range.", theme.EmptyTextBrush, 12.0, LeftPadding, TopPadding + AxisHeight + 8.0);
			return;
		}
			...
			.OrderBy(group => group.Min(item => item.StartFrame)))
		{
			...
			DrawText(context, threadRows.Key, theme.TextBrush, 10.0, labelRect.X + 4.0, labelRect.Y + 1.0);
			...
			foreach (ThreadTimelineScopeRow row in threadRows.OrderBy(item => item.StartFrame).ThenBy(item => item.Depth))
			{
				...
				context.DrawRectangle(null, ReferenceEquals(row, m_HoveredScope) ? theme.HoverPen : theme.BorderPen, barRect);
				if (barRect.Width > 58.0)
				{
					DrawText(context, row.Name, theme.TextBrush, 9.0, barRect.X + 3.0, barRect.Y);
				}
			}
	...
	private void DrawSelection(DrawingContext context, TimelineDrawingTheme theme, double left, double right, double top, double bottom, int startFrame, int endFrame)
	{
		...
		context.DrawLine(theme.SelectionPen, new Point(x, top), new Point(x, bottom));
	}
	...
	private static void DrawAxis(DrawingContext context, TimelineDrawingTheme theme, double left, double right, double top, int startFrame, int endFrame)
	{
		...
		for (int i = 0; i <= 4; ++i)
		{
			...
			DrawText(context, frame.ToString(CultureInfo.InvariantCulture), theme.MutedTextBrush, 9.0, x + 2.0, top);
		}
	}
